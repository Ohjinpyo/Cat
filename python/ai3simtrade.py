...
def create_table_if_not_exists(name):
    try:
        ...
        CREATE TABLE IF NOT EXISTS {name}ai3simtrade (
...
def reboot_table_if_exists(name):
    try:
        ...
        cursor = connection.cursor()
        query = f"DELETE FROM {name}ai3simtrade"
...
def auto_trade(username, key, secret, symbol, timeframe):
    try:
        ...
        while True:
            ...
            cursor = connection.cursor()
            query = "SELECT ai3si FROM User WHERE username = %s"
            ...
            if position is None:
                ...
                if position == 'long':
                    if df['close'].iloc[-2] >= entry_price * (1 + profit_ratio / 100) or df['close'].iloc[-2] <= entry_price * (1 - loss_ratio / 100):
                        ...
                        cursor = connection.cursor()
                        query = f"INSERT INTO {username}ai3simtrade (position, entryTime, entryPrice, exitTime, exitPrice, contract, profit, profitRate, deposit) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
