...
def preprocessing(df, windows=10):
    ...
    df['Rsi_label'] = 0
    df.loc[(df['Rsi'] < 0) & (df['Rsi_next'] > 0), 'Rsi_label'] = 1
    df.loc[(df['Rsi'] > 0) & (df['Rsi_next'] < 0), 'Rsi_label'] = 2
...
def undersample_data(df):
    ...
    X_resampled, y_resampled = under_sampler.fit_resample(df.drop(columns=['Macd_label', 'Rsi_label']), df['Macd_label'])
    df_resampled = pd.DataFrame(X_resampled, columns=[
        'Rsi_windows', 'Macd_hist_windows', 'Macd_windows', 'Macd_signal_windows',
        'Open_windows', 'High_windows', 'Low_windows', 'Close_windows', 'Volume_windows'])
    df_resampled['Macd_label'] = y_resampled
    return df_resampled
...
print(df_TSLA_processed['Macd_label'].value_counts())
...
y_combined = df_combined['Macd_label']
...
y_test_btc = df_btc_processed['Macd_label']
